...
async function ProjRow({ project }: { project: Project }) {
	...
	return (
		<TableTr id={project.projects_id.toString()} className="pl-2 pb-4 mb-4 text-xl">
			...
			<TableTd className={"project"}>
				<Link href={link_href}>{project.projects_name}</Link>
			</TableTd>
			<TableTd className={"client"}>
				<Link href={link_href}>{project.clients_name}</Link>
			</TableTd>
		</TableTr>
	);
}
...
async function get_relevant_projects(): Promise<Project[]> {
	...
	const data = projects_data
		...
		.sort(
			(a, b) =>
				Date.parse(a.project_dates_deliver_start) -
				Date.parse(b.project_dates_deliver_start),
		);
	return data;
}
		...
		<Card className="shadow-lg bg-white-500 m-auto md:w-3/4 w-full h-fit">
			...
			<CardSection>
				...
				>
					...
					<TableTbody>
						{relevant_projects.map((v) => (
							<ProjRow key={v.projects_id} project={v} />
						))}
					</TableTbody>
				</Table>
			</CardSection>
